...
*FIXME*: `nginx.conf` keeps changing... to prevent this and avoid the
really annoying redirect to `https`, make `/etc/nginx/conf.d/local.conf`
immutable:
...
==== Mutt configuration for native linux
...
==== Mutt configuration for MSYS2 on Windows

...
The fpm module systemd unit has

```
ProtectSystem=full
```

in `/etc/systemd/system/multi-user.target.wants/php-fpm.service`


which prevents `/usr` to be writeable. This will break almost all PHP applications.
To allow this directory to be writeable, add

```
ReadWriteDirectories=/usr/share/nginx/
```

for example, or whatever you need and restart with
